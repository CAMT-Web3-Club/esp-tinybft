                                ...
                                tbft_seqno_t last_executed)
{
    tbft_seqno_t old_head = ar->head;
    ar->head = new_head;
    ar->head_idx = (int)((ar->head_idx + (int)(new_head - old_head)) & ar->mask);

    for (tbft_seqno_t seqno = new_head;
         seqno < new_head + TBFT_WINDOW_SIZE;
         seqno++) {
        tbft_agreement_slice_t *sl = tbft_ar_slice(ar, seqno);
        if (seqno <= last_executed) {
            /* Keep Pre_prepare request body, but clear old view signatures/certificates */
